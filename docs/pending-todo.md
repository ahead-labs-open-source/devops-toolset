...
- [ ] Fix README badges: identify broken ones and update URLs/paths (CI, coverage, PyPI, license, etc.) so they render correctly on GitHub.
  - Current observations (2025-12-23):
    - README currently references GitHub Actions workflows `ci.yml` and `cd.yml`, but there is no `.github/` directory in the repo yet, so those badges/links are expected to be broken until workflows are added.
    - Azure Pipelines definitions/templates currently live under `devops_toolset/.devops/` (not at repo root).
  - [ ] List which badges are currently broken and why (repo/branch/workflow renamed, outdated paths, etc.).
  - [x] Update badges for `main` and/or `feature/*` as appropriate.
...
- [ ] Review whether the `src/devops_toolset` layout is consistent with Poetry and Python packaging best practices.
  - [ ] Confirm whether Poetry is already being used for dependency and version management.
    ...
    - Current observation (2025-12-23): `pyproject.toml` build-system pins differ from `requirements.txt` (e.g., `setuptools==57.0.0` vs `setuptools==65.5.1`, and different `requests` constraints). Decide which file is authoritative and align versions accordingly.
...
- [ ] Migrate CI/CD from Azure DevOps Pipelines to GitHub Actions.
  - [ ] Inventory current Azure Pipelines definitions/templates (currently under `devops_toolset/.devops/`) and document what each job does.
