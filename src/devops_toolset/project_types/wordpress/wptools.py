...
def get_default_project_structure(structure_type: ProjectStructureType, token_replacements: dict = None) -> dict:
    ...
    if structure_type is structure_type.WORDPRESS:
        project_structure_path = pathlib.Path(
            wordpress_directory_path, "default_files", wp_constants.FileNames.DEFAULT_WORDPRESS_PROJECT_STRUCTURE)
    else:
        # type is type.THEME
        project_structure_path = pathlib.Path(
            wordpress_directory_path, "default_files", wp_constants.FileNames.DEFAULT_WORDPRESS_DEV_THEME_STRUCTURE)
...
def get_snippet_cloudfront():
    ...
    default_cloudfront_forwarded_proto_php_file_path: pathlib.Path = pathlib.Path.joinpath(
        current_path.parent, "default_files", wp_constants.FileNames.DEFAULT_CLOUDFRONT_FORWARDED_PROTO_PHP)
